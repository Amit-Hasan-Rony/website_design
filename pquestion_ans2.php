...
<?php
   ...
   $result=mysqli_query($conn,$sql);
       ...
       if (mysqli_num_rows($result) > 0) {
       	  $i=-1;
          while($row = mysqli_fetch_assoc($result)) {  //code for saving informtion into array................
               $i++;
  ...
  if(!empty($_POST['submit_a'])){
    $q_id=$_POST['q_number'];
    $user=$_SESSION['username'];  //code for inserting information into answer table...............................
    $ans=$_POST['give_ans'];
    $sql="INSERT INTO $answer_table (id,name,answer) VALUES ($q_id,'$user','$ans')";
    if(mysqli_query($conn,$sql)){
      echo "Inserted successfully";
    }else {
      echo "<p style=\"margin-left: 500px;color: red;\">Something Wrong with the Code not inserted</p>";
      $reload_flag="ture";
    }
  }
      ...
      echo "<div style=\"margin-left: 25px;margin-top: 25px;\">
   	  ...
   	  if(mysqli_num_rows($result)>0){
   	  	while ($row=mysqli_fetch_assoc($result)) {
                 ...
                 <p style=\"color: green;margin-left: 20px;margin-top:-35px;\">$a:   <strong style=\"margin-left: 70px;\">$b </strong></p></br>
            </div>"; 

   	  	}
   	  }
   }
    $flag2=$_SESSION['loginflag'];
  ...
  <div class="write_division" id="two">
    <ul >
      <li style="margin-left: 350px;">
        <form action="" method="post">
          <input type="text" name="q_number" style="margin-left: -145px;margin-top: -50px;margin-right: 45px;width: 170px;" placeholder="Enter question Number">
          <textarea rows="3" cols="60" name="give_ans" placeholder="                    Enter your Answer"></textarea>
          <input name="submit_a" style="margin-top: -45px;margin-left: 50px;background: blue" type="submit" value="Submit Answer">
        </form>
      </li>
    </ul>
  </div>
